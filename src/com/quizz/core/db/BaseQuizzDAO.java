...
public class BaseQuizzDAO {
	...
	String sqlQuery = "SELECT" + " (SELECT" + " COUNT(" + DbHelper.TABLE_SECTIONS + "."
		+ DbHelper.COLUMN_ID + ")" + " FROM " + DbHelper.TABLE_SECTIONS + " WHERE "
		+ DbHelper.TABLE_SECTIONS + "." + DbHelper.COLUMN_UNLOCKED + " = "
		+ Section.SECTION_UNLOCKED + ") AS sections_unlocked," + " COUNT("
		+ DbHelper.TABLE_SECTIONS + "." + DbHelper.COLUMN_ID + ") AS sections_total,"
		+ " (SELECT" + " COUNT(" + DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_ID + ")"
		+ " FROM " + DbHelper.TABLE_LEVELS + " " + " WHERE " + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_STATUS + " = " + Level.STATUS_LEVEL_CLEAR + ") AS levels_clear,"
		+ " COUNT(" + DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_ID
		+ ") AS levels_total," + " (SELECT" + " COUNT(" + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_ID + ")" + " FROM " + DbHelper.TABLE_LEVELS + " WHERE "
		...
		+ ") AS levels_easy_clear," + " (SELECT" + " COUNT(" + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_ID + ")" + " FROM " + DbHelper.TABLE_LEVELS + " WHERE "
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_DIFFICULTY + " = \""
		+ Level.LEVEL_EASY + "\"" + ") AS levels_easy_total," + " (SELECT" + " COUNT("
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_ID + ")" + " FROM "
		+ DbHelper.TABLE_LEVELS + " WHERE " + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_STATUS + " = " + Level.STATUS_LEVEL_CLEAR + " AND "
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_DIFFICULTY + " = \""
		+ Level.LEVEL_MEDIUM + "\"" + ") AS levels_medium_clear," + " (SELECT" + " COUNT("
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_ID + ")" + " FROM "
		+ DbHelper.TABLE_LEVELS + " WHERE " + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_DIFFICULTY + " = \"" + Level.LEVEL_MEDIUM + "\""
		+ ") AS levels_medium_total," + " (SELECT" + " COUNT(" + DbHelper.TABLE_LEVELS
		+ "." + DbHelper.COLUMN_ID + ")" + " FROM " + DbHelper.TABLE_LEVELS + " WHERE "
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_STATUS + " = "
		+ Level.STATUS_LEVEL_CLEAR + " AND " + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_DIFFICULTY + " = \"" + Level.LEVEL_HARD + "\""
		+ ") AS levels_hard_clear," + " (SELECT" + " COUNT(" + DbHelper.TABLE_LEVELS + "."
		+ DbHelper.COLUMN_ID + ")" + " FROM " + DbHelper.TABLE_LEVELS + " WHERE "
		+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_DIFFICULTY + " = \""
		+ Level.LEVEL_HARD + "\"" + ") AS levels_hard_total" + " FROM "
		+ DbHelper.TABLE_SECTIONS + " LEFT JOIN " + DbHelper.TABLE_LEVELS;
	...
	cursor.moveToFirst();
	stats.add(new Stat("Niveaux débloqués", cursor.getInt(cursor
		.getColumnIndex("sections_unlocked")), cursor.getInt(cursor
		.getColumnIndex("sections_total"))));
	stats.add(new Stat("Images trouvées", cursor.getInt(cursor.getColumnIndex("levels_clear")),
		cursor.getInt(cursor.getColumnIndex("levels_total"))));
	stats.add(new Stat("Trouvées en facile", cursor.getInt(cursor
		.getColumnIndex("levels_easy_clear")), cursor.getInt(cursor
		.getColumnIndex("levels_easy_total"))));
	stats.add(new Stat("Trouvées en moyen", cursor.getInt(cursor
		.getColumnIndex("levels_medium_clear")), cursor.getInt(cursor
		.getColumnIndex("levels_medium_total"))));
	stats.add(new Stat("Trouvées en difficile", cursor.getInt(cursor
		.getColumnIndex("levels_hard_clear")), cursor.getInt(cursor
		.getColumnIndex("levels_hard_total"))));
	cursor.close();
