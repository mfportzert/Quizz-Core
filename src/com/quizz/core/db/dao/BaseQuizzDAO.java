...
public class BaseQuizzDAO {
	...
	public void firstLaunchDbInit(List<Section> sections) {
		...
		for (Section section : sections) {
			...
			for (Level level : section.levels) {
				...
				levelValues.put(DbHelper.COLUMN_STATUS,
						Level.STATUS_LEVEL_UNCLEAR);
				levelValues.put(DbHelper.COLUMN_FK_SECTION, sectionInsertId);
				mDbHelper.getWritableDatabase().insert(DbHelper.TABLE_LEVELS,
						null, levelValues);
				...
				// BaseQuizzApplication.db().insert(DbHelper.TABLE_RESPONSES,
				// null, responseValues);
				// }
			}
		}
	...
	public List<Section> loadGame() {
		...
		String sqlQuery = "SELECT " + DbHelper.TABLE_SECTIONS + "."
				...
				+ " FROM " + DbHelper.TABLE_SECTIONS + " INNER JOIN "
		...
		while (!cursor.isAfterLast()) {
			...
			if (lastId != cursor.getInt(cursor
	...
	private Level cursorToLevel(Cursor cursor) {
		...
		level.firstHint = cursor.getString(cursor
				.getColumnIndex(DbHelper.COLUMN_HINT1));
		level.secondHint = cursor.getString(cursor
				.getColumnIndex(DbHelper.COLUMN_HINT2));
		level.thirdHint = cursor.getString(cursor
				.getColumnIndex(DbHelper.COLUMN_HINT3));
		level.fourthHint = cursor.getString(cursor
				.getColumnIndex(DbHelper.COLUMN_HINT4));
		return level;
	}
