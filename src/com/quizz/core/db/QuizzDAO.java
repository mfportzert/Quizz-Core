...
public enum QuizzDAO {
	...
	public void insertSection(Section section) {
		...
		for (Level level : section.levels) {
			...
			levelValues.put(DbHelper.COLUMN_DIFFICULTY, level.difficulty);
			levelValues.put(DbHelper.COLUMN_RESPONSE, level.response);
			levelValues.put(DbHelper.COLUMN_PARTIAL_RESPONSE, level.partial_response);
			levelValues.put(DbHelper.COLUMN_STATUS, Level.STATUS_LEVEL_UNCLEAR);
			levelValues.put(DbHelper.COLUMN_FK_SECTION, sectionInsertId);
			long levelInsertId = mDbHelper.getWritableDatabase().insert(DbHelper.TABLE_LEVELS,
					null, levelValues);
			Log.d("Quizz-Core - BaseQuizzDAO",
					"INFO [" + level.imageName + " : " + String.valueOf(level.hints.size()) + "]");
			for (Hint hint : level.hints) {
				...
				mDbHelper.getWritableDatabase().insert(DbHelper.TABLE_HINTS, null, hintValues);
			}
		}
	...
	public static Cursor getHintsCursor(Level level) {
		String sqlQuery = "SELECT " + DbHelper.TABLE_HINTS + "." + DbHelper.COLUMN_HINT + ", "
				+ DbHelper.TABLE_HINTS + "." + DbHelper.COLUMN_HINT_TYPE + ", "
				+ DbHelper.TABLE_HINTS + "." + DbHelper.COLUMN_UNLOCKED + " FROM "
				+ DbHelper.TABLE_HINTS + " WHERE " + DbHelper.COLUMN_FK_LEVEL + " = "
				+ String.valueOf(level.id);

		Cursor cursor = QuizzDAO.INSTANCE.mDbHelper.getWritableDatabase().rawQuery(sqlQuery, null);
		return cursor;
	}
	...
	public Cursor getSections() {
		...
		String sqlQuery = "SELECT " + DbHelper.TABLE_SECTIONS + "." + DbHelper.COLUMN_ID + ", "
				...
				+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_DIFFICULTY + ", "
				+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_RESPONSE + ", "
				+ DbHelper.TABLE_LEVELS + "." + DbHelper.COLUMN_LINK + " FROM "
				+ DbHelper.TABLE_SECTIONS + " LEFT JOIN " + DbHelper.TABLE_LEVELS + " ON "
	...
	public static Hint cursorToHint(Cursor cursor) {
		Hint hint = new Hint();
		hint.hint = cursor.getString(cursor.getColumnIndex(DbHelper.COLUMN_HINT));
		hint.type = cursor.getInt(cursor.getColumnIndex(DbHelper.COLUMN_HINT_TYPE));
		hint.isUnlocked = (0 == cursor.getInt(cursor.getColumnIndex(DbHelper.COLUMN_UNLOCKED)));
		return hint;
	}
